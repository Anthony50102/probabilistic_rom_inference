"""
04b — Conditional GP + Weak-Form Dual Constraint  (smooth bump test functions)

Same structure as 04, but replaces the indicator-window integral constraint with
a WSINDy-style weak form using smooth compactly-supported test functions

    ψ_k(t) = (1 - τ²)^p,    τ = (t - t_k)/r,   supp ⊂ [t_k - r, t_k + r]

Because ψ_k(t_k ± r) = 0, integration by parts gives
    ∫ ψ_k(t) (dX/dt) dt = - ∫ ψ_k'(t) X(t) dt
so the weak-form residual for each test function k and mode i is

    -∫ ψ_k'(t) X_i(t) dt   ≈   ∫ ψ_k(t) [f(X) Oᵀ]_i dt.

This is mathematically the strong-form equation tested against {ψ_k}, and
mathematically subsumes the indicator-window integral of 04 (recovered when
ψ_k is the indicator on [t_a, t_b]).  It puts the "ad hoc" integral constraint
on the principled WSINDy / weak-SINDy footing (Messenger & Bortz, 2020).

Bayesian model otherwise identical to 04:
  θ_GP ~ LogNormal(MLE, σ),   X(t) = K_* K⁻¹ y,   O ~ N(O_ls, γ|O_ls|).
Physics constraints (likelihood factors in ELBO):
  1. Derivative (strong form):  dX/dt ≈ f(X)O^T
  2. Weak form (smooth ψ_k):    -∫ψ_k' X dt ≈ ∫ψ_k f(X)O^T dt
  3. GP MLL:                    log p(y|θ_GP)
...
MODEL_PARAMS = dict(
    ...
    NUM_TEST_FUNCS=None,     # if None, uses num_eval_points // WINDOW_SIZE
    BUMP_RADIUS_FRAC=None,   # if None, radius = WINDOW_SIZE * dt_eval (matches 04)
)
...
def build_model(
    ...
    bump_p=6, num_test_funcs=None, bump_radius_frac=None,
):
    """Build the conditional GP + weak-form NumPyro model.

    Test functions are bump polynomials ψ_k(τ) = (1 - τ²)^p with
    τ = (t - t_k) / r, support [t_k - r, t_k + r], smoothly vanishing
    at the endpoints so integration by parts has no boundary term.
    """
    ...
    # place bump centres so that supports cover the interval (allow overlap)
    centres = np.linspace(time_eval[0] + radius, time_eval[-1] - radius,
                          num_test_funcs)

    psi_list = []          # ψ_k evaluated on the eval grid (zero outside support)
    psi_dot_list = []      # ψ'_k evaluated on the eval grid
    int_psi_sq_list = []   # ∫ ψ_k² dt   — used for noise-variance scaling
    ...
    for tc in centres:
        ...
        psi_d_vals = np.where(in_supp,
            ...
            0.0)
        # zero out endpoints exactly (numerical hygiene)
        psi_vals[~in_supp] = 0.0
        psi_d_vals[~in_supp] = 0.0
        psi_list.append(jnp.array(psi_vals))
        psi_dot_list.append(jnp.array(psi_d_vals))
        # Riemann/trapezoid weights
        w = np.ones_like(time_eval) * dt_eval
        w[0] *= 0.5
        w[-1] *= 0.5
        int_psi_sq_list.append(float(np.sum(w * psi_vals ** 2)))

    psi_arr = jnp.stack(psi_list)        # (K, num_eval_points)
    psi_dot_arr = jnp.stack(psi_dot_list) # (K, num_eval_points)
    int_psi_sq_arr = jnp.array(int_psi_sq_list)
    # trapezoid weights as a single array
    trap_w = np.ones_like(time_eval) * dt_eval
    trap_w[0] *= 0.5
    trap_w[-1] *= 0.5
    trap_w_jnp = jnp.array(trap_w)
    ...
    def model(gamma=2.0, gamma2=10.0, jitter=1e-4):
        ...
        # Operator dynamics: f(X_eval) @ O^T  (shape: num_eval_points × num_modes)
        f_Xi = rom.model._assemble_data_matrix(Xs_eval, inputs=None) @ O.T

        # CONSTRAINT 1: Derivative matching (strong form)
        if deriv_weight > 0:
            for i in range(num_modes):
        ...
        # CONSTRAINT 2: Weak form against smooth bump test functions
        # For each ψ_k:  -∫ψ_k'·X dt  ≈  ∫ψ_k·f(X)Oᵀ dt
        if integral_weight > 0:
            # observed (integration-by-parts) side: Xs_eval is (num_modes, num_eval)
            # weak_obs[i, k] = -Σ_j w_j ψ_k'(t_j) X_i(t_j)
            wpsi_dot = trap_w_jnp[None, :] * psi_dot_arr             # (K, num_eval)
            weak_obs = -(Xs_eval @ wpsi_dot.T)                       # (num_modes, K)
            wpsi = trap_w_jnp[None, :] * psi_arr                     # (K, num_eval)
            weak_pred = (f_Xi.T @ wpsi.T)                            # (num_modes, K)
            # constraint std: noise scale times sqrt(∫ψ² dt)
            constraint_std = jnp.sqrt(gamma2 * int_psi_sq_arr)       # (K,)
            for i in range(num_modes):
                numpyro.factor(f"weakform_{i}",
                    integral_weight * jnp.sum(
                        dist.Normal(weak_pred[i], constraint_std).log_prob(weak_obs[i])))
...
def run_experiment(schema):
    ...
    model, time_eval = build_model(
        ...
        bump_p=p['BUMP_P'],
        num_test_funcs=p['NUM_TEST_FUNCS'],
...
def main(schema_names=None):
    ...
    print("=" * 70)
    print("04b — Conditional GP + Weak-Form (smooth bump test fns) — Euler")
    print("=" * 70)
    ...
    # Summary table
    print(f"\n\n{'='*80}")
    print(f"SUMMARY — Conditional GP + Weak-Form (Euler)  [bump p={MODEL_PARAMS['BUMP_P']}]")
    print(f"{'='*80}")
