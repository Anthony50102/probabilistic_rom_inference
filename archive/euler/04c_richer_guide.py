"""
04c — Conditional GP + Dual Constraint with configurable variational guide

Identical model to 04_conditional_integral.py, but the SVI guide family is
chosen at runtime so we can test whether richer posteriors close the gap
between empirical CI coverage (~62% on dense_high_noise) and the 90% target.

Available guides:
  - normal       : AutoNormal (mean-field diagonal Gaussian, default in 04)
  - lowrank      : AutoLowRankMultivariateNormal (paper's Stage-3 recommendation)
  - multivariate : AutoMultivariateNormal (full covariance)

Usage:
    python 04c_richer_guide.py normal       dense_high_noise
    python 04c_richer_guide.py lowrank      dense_high_noise
    python 04c_richer_guide.py multivariate dense_high_noise
    python 04c_richer_guide.py lowrank      # all 3 regimes with one guide
"""
...
MODEL_PARAMS = dict(
    ...
    SEED=42,
    GUIDE='normal',          # overridden by CLI; one of {normal,lowrank,multivariate}
    LOWRANK_RANK=8,          # rank for AutoLowRankMultivariateNormal
)

GUIDE_CHOICES = ('normal', 'lowrank', 'multivariate')


def _build_guide(model, name, init_values, rank):
    init_fn = init_to_value(values=init_values)
    if name == 'normal':
        return autoguide.AutoNormal(model, init_loc_fn=init_fn)
    if name == 'lowrank':
        return autoguide.AutoLowRankMultivariateNormal(
            model, init_loc_fn=init_fn, rank=rank)
    if name == 'multivariate':
        return autoguide.AutoMultivariateNormal(model, init_loc_fn=init_fn)
    raise ValueError(f"Unknown guide {name!r}; choose from {GUIDE_CHOICES}")
...
def run_experiment(schema):
    ...
    guide = _build_guide(model, p['GUIDE'], init_values, p['LOWRANK_RANK'])
    print(f"  Guide: {p['GUIDE']}" +
          (f" (rank={p['LOWRANK_RANK']})" if p['GUIDE'] == 'lowrank' else ""))
...
def plot_results(result, save_dir=None):
    ...
    prefix = f"04c_{MODEL_PARAMS['GUIDE']}_{schema['name']}"
...
def save_predictions(result, save_dir=None):
    ...
    method_name = f"04c_richer_guide_{MODEL_PARAMS['GUIDE']}"
...
def main(schema_names=None):
    ...
    print("=" * 70)
    print(f"04c — Conditional GP + Dual Constraint — Guide: {MODEL_PARAMS['GUIDE']}")
    print("=" * 70)
...
if __name__ == "__main__":
    args = sys.argv[1:]
    if args and args[0] in GUIDE_CHOICES:
        MODEL_PARAMS['GUIDE'] = args[0]
        args = args[1:]
    elif args and args[0] not in [s['name'] for s in SCHEMAS]:
        print(f"First arg must be guide ({'/'.join(GUIDE_CHOICES)}) or schema name.")
        sys.exit(1)
