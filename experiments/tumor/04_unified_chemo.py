...
04_unified_chemo.py — Marginalised-O × Weak-Form Bayesian OpInf (Tumor + Chemo).
    Tumor Growth WITH chemotherapy (input-driven, cABN operators).  Uses a
    fixed POD mode count with spectrum-anchored GP-hyperparameter priors +
    SVI (inline with the euler/burgers single-trajectory cases — no MLE).

This is the chemotherapy counterpart of ``04_unified.py``.  The reduced model
is now input-driven,

    dq̂/dt = ĉ + Â q̂ + B̂ α(t) + N̂ [α(t) ⊗ q̂],

where α(t) is the scalar chemo cell-death rate.  The B̂ (pure-input) term is
required because the POD basis is mean-centred: the physical forcing −α(t)·u
projects to a constant-in-state input term.  The quadratic Ĥ term is dropped
(unidentifiable from a single trajectory, q⊗q ~ magnitude²).

The marginalised-O + weak-form machinery is UNCHANGED from the autonomous case:
because α(t) is fixed data (not a sampled variable), the reduced dynamics
f(X, α) O^T are still linear in O, so the conditional posterior p(O | θ, data)
remains Gaussian and closed-form.  The only difference from ``04_unified.py`` is
that the design matrix ``f(X)`` is assembled WITH the chemo input,
``_assemble_data_matrix(Xs, inputs=inputs_eval)``.

For each ROM mode i, the likelihood uses two covariance-aware blocks:

    derivative rows (T_eval):
        μ_z_i(t_j)           ≈ [f(X, α) Oᵀ]_i(t_j)
        Σ_D,i                = Σ_z,i + γ² I

    weak-form rows (K):
        ∫ψ_k(t) μ_z_i(t) dt  ≈ ∫ψ_k(t) [f(X, α)Oᵀ]_i(t) dt
...
Prerequisite:
    python generate_fom_data_chemo.py     # writes the chemo FOM cache

Usage
-----
    python 04_unified_chemo.py                  # run all 3 regimes
    python 04_unified_chemo.py dense_low_noise  # run one regime
"""
...
import config
from config import (
    Basis, ChemoReducedOrderModel, load_chemo_fom_data,
    make_jax_input_func,
)
from core import (
...
MODEL_PARAMS = dict(
    NUM_MODES=4,           # fixed POD mode count (override with NUM_MODES env)
    GAMMA2=0.035,
    DERIV_WEIGHT=1.0,
    WEAKFORM_WEIGHT=8.0,
    MLL_WEIGHT=0.1,
    ...
    REGULARIZER=0.1,       # Tikhonov ridge for the cABN LS prior fit
    GP_PRIOR_SCALE=0.03,
    OPERATORS="cABN",      # const + linear + pure-input B + state-input N
    SEED=42,
)
...
# Headline chemo FOM cache (sparse 5-dose schedule, sensitivity 0.5, dose ×1.0).
# Matches 05_neural_ode_chemo so the two methods use identical data. Override
# with CHEMO_FOM_PATH=/abs/path for dose/schedule ablations.
...
def build_model(rom, num_modes, time_sampled, snapshots_comp,
                ...
                input_func=None):
    """Build marginalised-O + weak-form Bayesian model (chemo, input-driven).

    Parameters
    ----------
    input_func : callable t -> jnp.ndarray, shape (1,)
        JAX-traceable chemo input α(t).  Tabulated once on the internal
        ``time_eval`` grid and fed into the reduced-model data matrix as a
        fixed (non-sampled) input.  Because α(t) is data, f(X, α) stays
        linear in O and the closed-form O marginalisation is unchanged.
    ...
    # ── Chemo input α(t) tabulated at the eval grid (fixed data) ─────────
    if input_func is None:
        raise ValueError("build_model (chemo) requires a JAX input_func α(t).")
    inputs_eval_jnp = jnp.stack(
        [jnp.reshape(input_func(float(t)), ()) for t in time_eval]
    )[None, :]                                            # (1, num_eval_points)
    ...
    # Operator-block structure derived from the fitted operators (const,
    # linear, and — if present — quadratic). One learned prior scale per block.
    _col_blocks = []
    ...
    #   "block_hier" : hierarchical/ARD per-block prior. Each operator block
    #                  (const, linear, [quad]) gets a LEARNED scale τ_b with a
    #                  broad hyperprior; O is still marginalised analytically
    #                  given τ. The marginal likelihood (Bayesian Occam) sets the
    #                  block scales from the data — no hand-tuning. See
    #                  euler/04_unified.py for the analysis.
    ...
    def _build_AyP(ells, sig2s, nus, gamma2):
        ...
        f_X = rom.model._assemble_data_matrix(Xs, inputs=inputs_eval_jnp)
...
def run_experiment(schema, p=None):
    ...
    # ── Data: chemo FOM cache + matching α(t) input function ─────────────
    t_pred = np.linspace(TRAINING_SPAN[0], config.PREDICTION_DAYS, neval)
    fom, t_full, true_states, t_samp, snaps_noisy, ifn, chemo_meta = \
        load_chemo_fom_data(
            CHEMO_FOM_PATH, t_pred, TRAINING_SPAN, nsamp, noise, seed=p['SEED'])

    # JAX-traceable α(t): pre-tabulate on a dense grid over the full pred span.
    ...
    # ── POD basis (fixed mode count, inline with euler/burgers) ──────────
    # No MLE / GP-SNR mode selection: we use a fixed number of modes and let
    # the spectrum-anchored GP-hyperparameter priors + SVI in build_model do
    # all the GP work, exactly as the single-trajectory euler/burgers cases.
    # Override the count with NUM_MODES=<n> in the environment if needed.
    # Fit POD on CLEAN snapshots (matches 05_neural_ode_chemo) so the basis
    # itself is noise-free; only the GP/operator inputs are noisy.
    ...
    print(f"  Using {nmodes} modes  (POD energy: {basis.cumulative_energy:.4%})")

    # cABN ROM: const + linear + pure-input chemo (B) + state-input chemo (N).
    # The B term is REQUIRED because POD is mean-centred: the physical forcing
    # −α(t)·u projects to a pure input term Bα. The quadratic H is dropped
    # (unidentifiable from a single trajectory with reduced-state magnitude²).
    _operators = p.get('OPERATORS', os.environ.get("OPERATORS", "cABN"))
    _operators = os.environ.get("OPERATORS", _operators)
    # Per-block Tikhonov ridge (c, A, B, N) — one shared level.
    _reg = p['REGULARIZER']
    _ncols = {'c': 1, 'A': nmodes, 'B': 1, 'N': nmodes}
    block_reg_diag = np.concatenate(
        [np.full(_ncols[ch], _reg) for ch in _operators])
    rom = opinf.ROM(
        basis=basis,
        ddt_estimator=opinf.ddt.NonuniformFiniteDifferencer(t_samp),
        model=ChemoReducedOrderModel(
            operator_string=_operators,
            solver=opinf.lstsq.TikhonovSolver(regularizer=np.diag(block_reg_diag))))
    # Inputs at training samples: scalar α(t) as (1, nsamp).
    ...
    print(f"  Operator shape: {rom.model.operator_matrix.shape} ({_operators})")
    ...
    if 'log_tau_block' in post:
        tau_block_s = jnp.exp(jnp.asarray(post['log_tau_block']))  # (npost, 2)
        _tb_mean = np.exp(np.asarray(post['log_tau_block']).mean(0))
        print(f"  learned τ_block (per operator): "
    ...
    t_pred = np.linspace(TRAINING_SPAN[0], config.PREDICTION_DAYS, 400)
    Os, Xs, rom_solves = generate_rom_predictions(
        ...
        state0_samples=state0_samples, input_func=ifn_jax)
    ...
    # ── Persist results (schema matches 04_unified_chemo) ─────────
    ...
    alpha_pred = np.array([float(np.asarray(ifn_jax(t)).ravel()[0]) for t in t_pred])
    np.savez(os.path.join(out_dir, f'04_unified_chemo{os.environ.get("OUTPUT_SUFFIX","")}.npz'),
    ...
    return {
        ...
        'snaps_comp': snaps_comp, 'snaps_noisy': snaps_noisy,
        'true_comp': true_comp,
        't_full': t_full, 't_pred': t_pred, 't_samp': t_samp,
        'training_span': TRAINING_SPAN, 'num_modes': nmodes,
        'true_states': true_states, 'basis': basis,
        'chemo_input_func': ifn_jax, 'chemo_meta': chemo_meta,
        'alpha_pred': alpha_pred, 'operators': _operators,
    }


# =============================================================================
# Plotting (standard set: rom trajectories, loss, full-order error, traces)
# =============================================================================
def plot_regime(result, save_dir=None):
    """Standard diagnostic figures for one chemo regime.

    Mirrors the 05_neural_ode_chemo output layout so the two methods are
    directly comparable: reduced-coordinate trajectories with 5–95% bands,
    the SVI/NUTS loss, the full-order error decomposition, and operator
    posterior traces.
    """
    if save_dir is None:
        save_dir = FIGURE_DIR
    ...
    dose_days = np.asarray(result['chemo_meta']['dose_days'])
    ...
    if len(rom_solves) > 0:
        ...
        for i in range(nmodes):
            ...
            ax.scatter(t_samp, snaps_comp[i], s=10, color='tab:blue',
                       alpha=0.4, label='Noisy obs', zorder=4)
    ...
    # ── Full-order error decomposition ───────────────────────────────────
    if len(rom_solves) > 0:
        try:
            fig_e, _ = plot_full_order_error(
                rom_solves=np.array(rom_solves), basis=result['basis'],
                true_states=result['true_states'], time_domain_full=t_full,
...
def main(schema_names=None):
    ...
    print("=" * 78)
    print("04_unified_chemo — Marginalised-O × Weak-Form Bayesian OpInf "
          "(Tumor + Chemo)")
    print("=" * 78)
    print(f"γ²={MODEL_PARAMS['GAMMA2']}  σ_O={MODEL_PARAMS['SIGMA_O']}  "
          f"bump_p={MODEL_PARAMS['BUMP_P']}  ops={MODEL_PARAMS['OPERATORS']}  "
          f"lr={MODEL_PARAMS['LEARNING_RATE']}  steps={MODEL_PARAMS['NUM_STEPS']}")

    results = []
    for schema in schemas:
        r = run_experiment(schema)
        plot_regime(r, FIGURE_DIR)
        results.append(r)

    # Summary table
    print(f"\n\n{'='*82}")
    print(f"SUMMARY — Marg-O × Weak-Form (Tumor + Chemo)")
    print(f"{'='*82}")
