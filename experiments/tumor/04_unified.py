"""
04_unified.py — Marginalised-O × Weak-Form Bayesian OpInf (Tumor).
    Tumor Growth (autonomous, cA operators).  Uses adaptive POD via SNR
    truncation: probe with extra modes, keep only those with σ²/ν > 10.
...
MODEL_PARAMS = dict(
    NUM_MODES=4,           # max — adaptive POD may reduce
    GAMMA2=0.035,
    DERIV_WEIGHT=1.0,
    WEAKFORM_WEIGHT=8.0,
    MLL_WEIGHT=0.1,
...
def run_experiment(schema, p=None):
    ...
    # ── Data: tumor uses load_fom_data (precomputed FOM cache) ───────────
    t_pred = np.linspace(TRAINING_SPAN[0], config.PREDICTION_DAYS, neval)
    fom, t_full, true_states, t_samp, snaps_samp = load_fom_data(
        t_pred, TRAINING_SPAN, nsamp, noise)

    # ── Adaptive POD: probe with extra modes, keep high-SNR ones ─────────
    max_modes = nmodes + 4
    basis_probe = Basis(num_vectors=max_modes)
    basis_probe.fit(snaps_samp)
    snaps_comp_probe = basis_probe.compress(snaps_samp)
    Ls_p, Vs_p, Ns_p, _ = fit_gp_hyperparameters_mle(
        t_samp, snaps_comp_probe, verbose=False)
    snr_thresh = p.get('SNR_THRESHOLD', 10.0)
    eff = 0
    for j in range(max_modes):
        snr = Vs_p[j] / max(Ns_p[j], 1e-10)
        ok = "✓" if snr > snr_thresh else "✗"
        print(f"  Probe mode {j}: σ²={Vs_p[j]:.4f}, ν={Ns_p[j]:.6f}, "
              f"SNR={snr:.1f} {ok}")
        if snr > snr_thresh:
            eff = j + 1
    eff = max(eff, 2)
    nmodes = min(eff, p['NUM_MODES'])
    print(f"  → Using {nmodes} modes (SNR threshold={snr_thresh})")
    ...
    # Operators: cA (default, autonomous const+linear). Set OPERATORS=cAH to
    # add a quadratic term — under-determined, but the block_hier ARD prior
    # regularizes it automatically (diagnostic for whether quad expressivity
    # improves accuracy/coverage on tumor).
    _operators = os.environ.get("OPERATORS", "cA")
    rom = opinf.ROM(
        basis=basis,
        ddt_estimator=opinf.ddt.NonuniformFiniteDifferencer(t_samp),
        model=JaxCompatibleModel(operators=_operators,
                                 solver=opinf.lstsq.L2Solver(regularizer=p['REGULARIZER'])))
    rom.fit(states=snaps_samp)

    # GP-MLE: reuse probe results for the kept modes (faster, identical)
    Ls = Ls_p[:nmodes]
    Vs = Vs_p[:nmodes]
    Ns = Ns_p[:nmodes]
...
def main(schema_names=None):
    ...
    print("=" * 78)
    print("04_unified — Marginalised-O × Weak-Form Bayesian OpInf (Tumor)")
    print("=" * 78)
    ...
    # Summary table
    print(f"\n\n{'='*82}")
    print(f"SUMMARY — Marg-O × Weak-Form (Tumor)")
    print(f"{'='*82}")
