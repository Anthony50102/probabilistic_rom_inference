...
def build_model(rom, num_modes, time_sampled, snapshots_comp,
    ...
    def _single_gp_conditional(ell, sig2, nu, y_i):
        ell2 = ell ** 2
        # Stronger jitter for high-variance modes to keep K_tt well-conditioned
    ...
    def _per_mode_evidence(A, y_i, prec_i, m, inv_sigma_O2, log_sigma_O2):
        ...
        Aw = A * prec_i[:, None]
        M_i = A.T @ Aw                                          # (m, m)
        # Symmetrise to avoid numerical asymmetry blocking Cholesky
        M_i = 0.5 * (M_i + M_i.T)
        # Trace-based ridge ensures SPD for ill-conditioned A
