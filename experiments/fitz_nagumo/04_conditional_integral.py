...
Operators: cAHBN (constant, linear, quadratic, input, state×input)
  - FitzHugh-Nagumo has a Neumann BC input and state×input coupling

Data regime:
  Dense data, medium noise (200 samples, 3% noise, 400 eval points)
...
def _generate_fn_predictions(O_samples, rom, q0, time_eval, input_func, max_samples=200):
    """Generate ROM predictions using scipy solve_ivp (diffrax has issues with input_func)."""
    solves = []
    n = min(len(O_samples), max_samples)
    indices = np.linspace(0, len(O_samples) - 1, n, dtype=int)
    for idx in indices:
        rom.model._extract_operators(np.array(O_samples[idx]))
        def rhs(t, state, _rom=rom, _ifunc=input_func):
            return np.array(_rom.model.rhs(t, state, _ifunc))
        try:
            sol = solve_ivp(rhs, [float(time_eval[0]), float(time_eval[-1])],
                          q0, t_eval=time_eval, method='RK45', max_step=0.05)
            if sol.success and np.all(np.isfinite(sol.y)):
                solves.append(sol.y)
        except Exception:
            pass
    return np.array(O_samples[indices]), np.array(solves) if solves else np.empty((0, len(q0), len(time_eval)))
...
def build_model(
    ...
    """Build the conditional integral NumPyro model for FitzHugh-Nagumo."""
    ...
    # Precompute inputs at eval points for JAX (must be done outside model fn)
    inputs_eval = jnp.array(
        [config.ReducedOrderModel.input_func_jax(float(t)) for t in time_eval]
    ).reshape(1, -1)
    ...
    def model(gamma=2.0, gamma2=1.0, jitter=1e-4):
        ...
        # Operator dynamics: f(X_eval) @ O^T — with inputs for FN
        f_Xi = rom.model._assemble_data_matrix(Xs_eval, inputs=inputs_eval) @ O.T
...
def run_experiment(schema):
    ...
    # Build ROM with input operators
    inputs_for_fit = np.array(
        [config.ReducedOrderModel.input_func(t) for t in t_samp]
    ).reshape(1, -1)
    rom = opinf.ROM(
        basis=basis,
        ddt_estimator=opinf.ddt.NonuniformFiniteDifferencer(t_samp),
        model=JaxCompatibleModel(operators="cAHBN",
                                 solver=opinf.lstsq.L2Solver(regularizer=1e0)),
    )
    rom.fit(states=snaps_samp, inputs=inputs_for_fit)
    ...
    # LS operator (with inputs at eval points)
    ...
    inputs_ls = jnp.array(
        [config.ReducedOrderModel.input_func_jax(t) for t in t_eval_ls]
    ).reshape(1, -1)
    D = np.array(rom.model._assemble_data_matrix(jnp.array(X_mle), inputs=inputs_ls))
    DtD = D.T @ D
    ...
    Os, rom_solves = _generate_fn_predictions(
        O_samples=O_samp,
        rom=rom, q0=snaps_comp[:, 0],
        time_eval=t_pred,
...
def main(schema_names=None):
    ...
    print("=" * 70)
    print("04 — Conditional GP + Dual Constraint — FitzHugh-Nagumo")
    print("=" * 70)
    ...
    # Summary table
    print(f"\n\n{'='*80}")
    print(f"SUMMARY — Conditional GP + Dual Constraint (FitzHugh-Nagumo)")
    print(f"{'='*80}")
