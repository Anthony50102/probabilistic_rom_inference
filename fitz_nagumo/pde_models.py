...
class FitzHughNagumo(_BasePDE):
    ...
    FitzHugh-Nagumo equations with time-varying Neumann boundary conditions:

        q_t = e q_xx + (-q^3 + 1.1 q^2 - 0.1 q - w + a) / e
        w_t = b q - g w + a

    with initial conditions q(x, 0) = w(x, 0) = 0 and boundary conditions

        q_x(0, t) = f(t),   q_x(1, t) = 0.
    ...
    spatial_domain : (N,) ndarray
        Spatial domain for all simulations.
    parameters : (float, float, float, float)
        Model parameters (a, b, g, e).
    """
    ...
    # Spatial domain and constant differential operators ----------------------
    @property
    def spatial_domain(self):
        """Spatial domain."""
        return self.__x
    ...
    def spatial_domain(self, space):
        """Set (or reset) the spatial domain."""
        dx = np.diff(space)
        if not np.allclose(dx, dx[0]):
            raise ValueError("nonuniform spatial domain not supported")
        ...
        self.__x = space
        self.__dx = dx[0]
        self.__N = 2 * (nx := space.size)

        # Construct matrices used by the full-order solver.
        dx2inv = 1 / (dx[0] ** 2)
        diags = np.array([1, -2, 1]) * dx2inv
        A = sparse.diags(diags, [-1, 0, 1], (nx, nx)).todok()
    ...
    def nx(self):
        ...
        return self.spatial_domain.size

    @property
    def N(self):
        """Degrees of freedom in the spatially discretized state."""
        return self.__N

    # System parameters and scaled differential operators ---------------------
    @property
    def parameters(self):
        r"""System parameters :math:`(\alpha,\beta,\gamma,\epsilon)`."""
        return self.__params

    @parameters.setter
    def parameters(self, params):
        """Set the parameters and construct the stiffness matrix."""
        self.__params = tuple(params)
        if len(params) != 4:
            raise ValueError("expected exactly 4 system parameters")

        eps = self.__params[-1]
        self.__A = (eps * self.__A1 - (0.1 / eps) * self.__Id).tocsr()

    @property
    def stiffness(self):
        """Stiffness matrix. Constructed when the spatial domain is set."""
        return self.__A
    ...
    def input_args(self, args):
        if len(args) != 2:
            raise ValueError("expected 2-tuple for input_args")
        if any(arg < 0 for arg in args):
            raise ValueError("input_args should be positive")
    ...
    @staticmethod
    def left_neumann_condition(t, a, b):
    ...
    def derivative(self, t: float, state: np.ndarray) -> np.ndarray:
        ...
        (2N,) ndarray
            State derivative at time t.
        """
        a, b, g, e = self.parameters
        q1, q2 = np.split(state, 2, axis=0)

        # fmt: off
        dq1dt = (self.stiffness @ q1) + (1.1 * (q1**2) - q1**3 - q2 + a) / e
        dq1dt[0] = dq1dt[0] - 2 * e * self.input_func(t) / self.dx
        dq2dt = b * q1 - g * q2 + a
        # fmt: on
        return np.concatenate([dq1dt, dq2dt], axis=0)

    def jacobian(self, t: float, state: np.ndarray) -> np.ndarray:
        """Calculate the state Jacobian."""
        _, b, g, e = self.parameters
        q1 = np.split(state, 2, axis=0)[0]

        dq11 = (
            self.stiffness
            + (sparse.diags(2.2 * q1) - sparse.diags(3 * q1**2)) / e
        )
        dq12 = -self.__Id / e
        dq21 = b * self.__Id
        dq22 = -g * self.__Id

        return sparse.bmat([[dq11, dq12], [dq21, dq22]])
    ...
    def solve(
        ...
        """Solve the model with scipy.integrate.solve_ivp(method="Radau").
        ...
        timepoints : (k,) ndarray
            Time domain over which to solve the equations.
        a : float or None
            First parameter for the Neumann boundary condition function.
        b : float or None
            Second parameter for the Neumann boundary condition function.
    ...
    def input_func(self, t):
        """Input function: left Neumann boundary condition with parameters
        specified by ``input_args``.
        """
        a, b = self.input_args
        return self.left_neumann_condition(t, a, b)
    ...
    def data_matrix(
        ...
        """Construct the data matrix for Operator Inference,

            [ 1 | Q^T | (Q ⊗ Q)^T | (Q ⊗ Q ⊗ Q)^T | U^T ]

        where Q is the low-dimensional ``states_`` and U is the ``inputs``.

        Parameters
        ----------
        states_ : (r, k) ndarray
            Training states, compressed to an r-dimensional manifold.
        inputs : (2, k) ndarray
            Training inputs corresponding to the states.
        """
        return np.vstack(
            (
                np.ones((1, states_.shape[1])),
                states_,
                opinf.operators.QuadraticOperator.datablock(states_),
                opinf.operators.CubicOperator.datablock(states_),
                inputs,
            )
        ).T
    ...
    def _param_labels(params):
        return ", ".join(
            [
                rf"$\alpha={params[0]:.3f}$",
                rf"$\beta={params[1]:.3f}$",
                rf"$\gamma={params[2]:.3f}$",
                rf"$\varepsilon={params[3]:.3f}$",
            ]
        )

    def plot_space(self, state, axes=None):
        """Plot variables q1(t=fixed, x) and q2(t=fixed, x) over space.

        Parameters
        ----------
        state : (N, k) ndarray
            Snapshot set to plot.
        axes : (plt.Axes, plt.Axes) or None
        ...
        q1, q2 = np.split(state, 2)

        axes[0].plot(self.x, q1)
        axes[1].plot(self.x, q2)
        axes[0].set_ylabel(r"$q_{1}(t_{j}, x)$")
        axes[1].set_ylabel(r"$q_{2}(t_{j}, x)$")
    ...
    def plot_time(
        ...
        **kwargs,
        ...
        """Plot q1 and q2 individually in time.
        ...
        axes : (plt.Axes, plt.Axes) or None
            Axes on which to draw. If ``None`` (default),
            a new figure is created.
        color : str or None
            Line colors. If ``None`` (default), choose the color based
        ...
        if color is None:
            colors = plt.cm.viridis_r(np.linspace(0.2, 1, nlines))
        else:
            colors = [color] * nlines
        xlocs = np.logspace(0, np.log10(self.nx - 1), nlines, dtype=int)

        if "lw" not in kwargs:
            kwargs["lw"] = 1
        for i, c in zip(reversed(xlocs), reversed(colors)):
            axes[0].plot(t, state[i, :], color=c, **kwargs)
            axes[1].plot(t, state[i + self.nx, :], color=c, **kwargs)

        axes[0].set_ylabel(r"$q_{1}(x_{j}, t)$")
        axes[1].set_ylabel(r"$q_{2}(x_{j}, t)$")
    ...
    def plot_phase(
        ...
        **kwargs,
        ...
        """Plot q1 versus q2 in time at several points in the domain.
        ...
        if color is None:
            colors = plt.cm.viridis_r(np.linspace(0.2, 1, nlocs))
        else:
            colors = [color] * nlocs
        xlocs = np.logspace(0, np.log10(self.nx - 1), nlocs, dtype=int)

        if "lw" not in kwargs:
            kwargs["lw"] = 1
        if "markevery" not in kwargs:
            kwargs["markevery"] = state.shape[1] // 40
        for i, c in zip(reversed(xlocs), reversed(colors)):
            ax.plot(
                state[i, :],
                state[i + self.nx, :],
        ...
        ax.set_xlabel(r"$q_{1}(x_{j}, t)$")
        ax.set_ylabel(r"$q_{2}(x_{j}, t)$")
        # ax.set_xlim(-0.5, 1.5)
        # ax.set_ylim(0, 0.2)
    ...
    def plot(self, state, t, nlines: int = 10):
        ...
        self.plot_time(state, t, nlines, [ax1, ax2])
        self.plot_phase(state, nlines, ax3)
        ax1.set_xticklabels("")

        return fig, [ax1, ax2, ax3]

    def plot_spacetime(self, state, t):
        """Plot the state in space-time.
        ...
        for qi, ax in zip(np.split(state, 2), axes):
            cdata = ax.pcolormesh(X, T, qi, shading="nearest", cmap="magma")
