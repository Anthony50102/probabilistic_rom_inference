...
def compute_full_order_errors(rom_solves, t_pred, basis, t_full, true_states):
    ...
    rank = rom_solves.shape[1]
    full_rank = true_comp.shape[0]
    if rank > full_rank:
        raise ValueError(f"Prediction rank {rank} exceeds basis rank {full_rank}")
    true_comp[rank:] = 0
    ...
    for i in range(rom_solves.shape[0]):
        coeffs = np.pad(rom_solves[i], ((0, full_rank - rank), (0, 0)))
        rom_full = basis.decompress(coeffs)
...
def error_comparison(methods, projection_error, t_pred, training_span,
    ...
    if has_proj:
        ...
        for m in methods:
            ...
            proj = _attr(m, "projection_error")
            if proj is None:
                proj = np.interp(mt, t_pred, projection_error)
            excess = np.maximum(med - proj, 1e-16)
...
def metrics_bars(methods, title, save_path, train_test_errors=None):
    ...
    for ax, key, title_ in zip(
            axes[:2], ("train", "pred"),
            ("Training-region Error", "Prediction-region Error")):
        if train_test_errors is None:
            _bars(ax, [_attr(m, f"{key}_error") for m in methods],
                  lambda v: f"{v:.3f}", "Relative L2 Error", title_)
            continue
        width = 0.38
        for offset, prefix, label, alpha, hatch in (
                (-width / 2, "", "Train ICs (avg)", 1.0, ""),
                (width / 2, "test_", "Test IC", 0.45, "//")):
            vals = [train_test_errors[label_][f"{prefix}{key}_l2"]
        ...
        ax.set(ylabel="Relative L2 Error", title=title_)
