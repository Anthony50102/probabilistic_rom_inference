...
def _metrics(result):
    keys = ("train_error", "pred_error", "stability_pct", "ci_coverage",
            "ci_width", "ci_cov_fit", "ci_cov_ext", "n_stable", "n_total")
    return {prefix + key: result[prefix + key]
            for prefix in ("", "test_") for key in keys if prefix + key in result}
...
class TargetResult:
    ...
    snapshots_comp: Optional[np.ndarray] = None
...
class RunResult:
    ...
    def from_flat(result, method_name):
        ...
        return RunResult(
            ...
            runtime=float(result.get("runtime", np.nan)), metrics=_metrics(result))
    ...
    def from_multi(result, method_name):
        ...
        all_times = result.get("all_t_samp")
        num_train = result.get(
            "num_train_ics", len(all_rs) - int("test_train_error" in result))
        ...
        for k in range(len(all_rs)):
            ...
            targets.append(TargetResult(
                ...
                t_samp=all_times[k] if all_times is not None else None,
                snapshots_comp=sn, is_training=k < num_train,
                train_error=result.get("all_train_errors", [np.nan] * len(all_rs))[k],
                pred_error=result.get("all_pred_errors", [np.nan] * len(all_rs))[k],
    ...
    def aggregate(self):
        """Preserve supplied metrics, otherwise average training targets."""
        if self.metrics:
            return dict(self.metrics)
        training = [t for t in self.targets if t.is_training]
        fin = [t for t in training if np.isfinite(t.train_error)]
        m = lambda k: float(np.mean([getattr(t, k) for t in fin])) if fin else float("nan")
        return dict(
            stability_pct=float(np.mean([t.stability_pct for t in training])),
    ...
    def save_npz(self, out_dir, suffix=""):
        ...
        Preserve all targets and observations; multi-IC files use indexed keys.
        """
        ...
        data = dict(
            t_pred=tgt.t_pred, t_full=tgt.t_full, runtime=self.runtime,
            num_modes=self.num_modes, training_span=np.array(self.training_span),
            losses=np.asarray(self.losses), basis_entries=np.asarray(self.basis.entries),
            **agg)
        multi = len(self.targets) > 1
        if multi:
            data.update(n_ics=len(self.targets),
                        num_train_ics=sum(t.is_training for t in self.targets),
                        eval_labels=np.array([t.label for t in self.targets]))
        for i, t in enumerate(self.targets):
            fields = dict(
                rom_solves=(np.asarray(t.rom_solves) if t.n_stable > 0
                            else np.empty((0, self.num_modes, len(t.t_pred)))),
                true_comp=t.true_comp, true_states=t.true_states,
                t_pred=t.t_pred, t_full=t.t_full,
                t_samp=t.t_samp if t.t_samp is not None else (self.t_samp if i == 0 else None),
                snaps_comp=t.snapshots_comp if t.snapshots_comp is not None
                else (self.snapshots_comp if i == 0 else None))
            suffix_i = f"_{i}" if multi else ""
            data.update({k + suffix_i: v for k, v in fields.items() if v is not None})
        if hasattr(self.basis, "shift_"):
            data["basis_shift"] = np.asarray(self.basis.shift_)
        if self.O_samples is not None:
            data["O_samples"] = np.asarray(self.O_samples)
        data.update({f"extra_{k}": v for k, v in self.extra.items() if np.ndim(v) <= 2})
        np.savez(path, **data)
        return path
...
class MethodData:
    ...
    projection_error: Optional[np.ndarray] = None
...
def load_comparison_npz(path, name, label=None, color=None):
    ...
    with np.load(path, allow_pickle=True) as d:
        return MethodData(
            ...
            rom_solves=d["rom_solves"] if "rom_solves" in d else d["rom_solves_0"],
            t_pred=d["t_pred"],
