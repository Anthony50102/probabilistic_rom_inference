...
# =============================================================================
# Convenience function for fitting multiple GPs
# =============================================================================
def fit_gaussian_processes(
    ...
    constant_bounds: tuple = (1e-5, 1e5),
    length_scale_bounds: tuple = (1e-5, 1e2),
    noise_level_bounds: tuple = (1e-16, 1e2),
    ...
    gp_regularizer : float >= 0
        Regularization hyperparameter for GP inference.
    constant_bounds : tuple
        Bounds for the constant kernel hyperparameter.
    length_scale_bounds : tuple
        Bounds for the RBF length scale.
    noise_level_bounds : tuple
        Bounds for the noise level.
    n_restarts_optimizer : int
    ...
    -------
    list of GP_RBFW
        Fitted Gaussian process regressors, one per state variable.
    """
    ...
    for stateindex in range(num_state_variables):
        varname = f"r_{stateindex+1:02d}"
        state_variable = snapshots_sampled[stateindex, :]
        ...
        with opinf.utils.TimedBlock(f"\nfitting GP model for state '{varname}'\n"):
            ...
            gp.fit(time_domain_sampled, state_variable)
            print(gp)
